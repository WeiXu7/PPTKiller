...
function Preview({ project, session, activeSlide, setActiveSlide }) {
    ...
    <section className="preview">
      <div className="previewTabs">
        ...
        <span className={`exportBadge ${exportPreviewState}`}>
          {exportPreviewState === "loading" ? "正在渲染导出预览" : exportPreviewState === "ready" ? "真实 PPT 预览" : exportPreviewState === "failed" ? "预览生成失败" : "草稿预览"}
        </span>
      </div>
        ...
        <div className="slideDetail">
          {selectedThumb ? (
            ...
            <div className="mainSlide">
              <span className="eyebrow">PPTKILLER · AGENT GENERATED</span>
              <h1>{selected?.title || project?.title}</h1>
              <h2>{project?.topic || "专业演示文稿"}</h2>
              <div className="slideVisual"><div className="visualLines"><i /><i /><i /></div><div className="visualOrb"><Sparkle weight="fill" /></div><div className="visualDots">{[1,2,3,4,5,6,7].map((n) => <i key={n} />)}</div></div>
              <footer>{project?.speaker_notes_enabled ? "已启用逐页演讲稿" : "未启用演讲稿"}　·　{project?.slide_count || 0} 页</footer>
            </div>
          )}
...
export function App() {
  ...
  async function createProject(payload) {
    ...
    try {
      ...
      setSession(started);
      notify(`项目已创建，${payload.files.length} 个文件上传完成`);
    } catch (error) {
      notify(error.message);
    } finally {
      setBusy(false);
      ...
      <main className="main">
        {project ? <>
          <header className="topbar">
            ...
            <div className="topActions"><button onClick={() => notify("团队分享将在后续版本接入")}><ShareNetwork /> 分享</button><button onClick={exportPpt}><DownloadSimple /> 导出PPT</button><button className="primary"><PencilSimple /> 打开编辑器 <CaretDown /></button></div>
          </header>
          <div className="content"><section className="runPanel"><Timeline steps={steps} busy={busy} onApprove={approve} onRevise={() => revise()} /></section><Preview project={project} session={session} activeSlide={activeSlide} setActiveSlide={setActiveSlide} /></div>
